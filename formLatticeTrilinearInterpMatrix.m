% Form an interpolation matrix that projects values on a lattice grid to
% arbitrary points using trilinear interpolation
% FUNCTION P = formLatticeTrilinearInterpMatrix(nodeX,nodeY,nodeZ,points)
% INPUT
%     nodeX, nodeY, nodeZ: define a lattice structure (eg. nodes of a 3D mesh)
%     points: an x|y|z matrix of the inquiry points
% OUTPUT
%     P: a sparse projection matrix; this matrix can be applied to a vector
%     holding values defined on the lattice grid
% NOTE
%     nodeX, nodeY, nodeZ can define any lattice structure, for example, can 
%     be the midpoints of edges or the centers of faces in a 3D mesh
